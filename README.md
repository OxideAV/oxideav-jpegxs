...
* High bit depth (`B[i] > 8`) beyond the round-118 lossless +
  ...
  path (`encode_planar_nlt_extended_highbd`) and the round-195
  Star-Tetrix path (`encode_planar_star_tetrix_highbd`): the only
  remaining Star-Tetrix high-bit-depth gap is lossy `q > 0` (the
  inner `encode_planar_inner_bd` already accepts `q > 0` with
  `Cpih = 3`, but `encode_planar_star_tetrix_highbd` pins `q = 0`
  for the round-195 lossless scope). Bit depths above 16 need a
