...
* High bit depth (`B[i] > 8`) beyond the round-118 lossless +
  ...
  (`encode_planar_nlt_quadratic_highbd`) and the round-193 NLT
  extended path (`encode_planar_nlt_extended_highbd`): Star-Tetrix
  (`Cpih = 3`) pre-distortion is still 8-bit-input specific (the
  forward Star-Tetrix lifting in `colour_transform::forward_star_tetrix`
  accepts `i32` coefficients, so widening is a matter of plumbing
  the `u16`-LE plane format through `encode_planar_star_tetrix` and
  selecting `Bw = B[i]` for the lossless case). The decoder's Annex G
  output already packs `u16` for any `B[i] ∈ 9..=16`. Bit depths
  above 16 need a wider plane format (`u32`).
