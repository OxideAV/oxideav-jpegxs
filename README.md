...
* High bit depth (`B[i] > 8`) beyond the round-118 lossless +
  ...
  sampled paths, and the round-181 NLT quadratic path
  (`encode_planar_nlt_quadratic_highbd`): Star-Tetrix (`Cpih = 3`)
  and **NLT extended** (`Tnlt = 2`, Annex G.5) pre-distortion are
  still 8-bit-input specific. NLT extended widening needs
  `build_extended_forward_lut`'s reconstructed-level table grown from
  `(1 << bc).min(257)` to the full `1 << bc` so the per-pixel inverse
  LUT can be built at `bc ∈ 9..=16` (at `bc = 16` that's a 65 536-
  entry `Vec<Option<u32>>` — feasible but worth measuring). The
  decoder's Annex G output already packs `u16` for any `B[i] ∈
  9..=16`, so widening the remaining encoder entry points is a
  matter of accepting `u16` planes and selecting `Bw` (round-181
  picks `Bw = 20` for NLT quadratic — the top of the Table A.8
  `{8, 18, 20}` set — to keep ≥ 4 bits of precision headroom over
  the sqrt at any `B[i]`). Bit depths above 16 need a wider plane
  format (`u32`).
