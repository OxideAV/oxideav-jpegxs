...
* `oxideav_jpegxs::pick_rp_for_target_bytes(width, height, nc, cpih,
  ...
  call — no internal model of the entropy coder, no oracle, no
  external library. Scope mirrors `encode_planar_rp` exactly: 4:4:4,
...
* Encoder round 219+: a **joint** rate-budget picker that
  simultaneously picks `q_slices` (the per-slice `Q[p]`, r212) and
  `R[p]` (the precinct refinement, r218) against a single byte
  budget. The two levers are orthogonal on the bitstream — per-slice
  `Q[p]` lives in each precinct's `Q[p]` byte and `R[p]` lives in
  each precinct's `R[p]` byte — so the joint picker can mix them on
  any picture. Round 218 lands the *single-axis* `R[p]` picker
  (`pick_rp_for_target_bytes` + `encode_planar_rp_target_bytes`,
  linear scan from `NL-1` down to `0` returning the first fit); the
  joint picker is the next refinement step. An `R[p]`-driven
  PSNR-optimizing **priority assignment** (instead of the plain
  band-index priorities `P[b] = b`) is a further refinement lever
  that's still open after the joint rate picker.
