...
* High bit depth (`B[i] > 8`) beyond the round-118 lossless +
  round-133 lossy 4:4:4 paths and the round-151 4:2:2 / 4:2:0 sub-
  sampled paths: Star-Tetrix (`Cpih = 3`) and NLT pre-distortion are
  still 8-bit-input specific. The decoder's Annex G output already
  packs `u16` for any `B[i] ∈ 9..=16`, so widening the remaining
  encoder entry points is a matter of accepting `u16` planes and
