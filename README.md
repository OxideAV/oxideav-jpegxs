...
* Encoder round 207+: per-band per-precinct Q rate-distortion
  optimization. Round 206 lands the **per-slice `Q[p]` override**
  primitive (`encode_planar_hsl_qslice`) so each slice can pick a
  different `Q[p]`; what's still open is a PSNR-driven *picker* that
  given a rate budget computes the per-slice `Q[p]` assignment. With
  round 115's `R[p]` refinement now in place, an `R[p]`-driven
  PSNR-optimizing priority assignment (instead of the plain band-index
  priorities) is a further refinement lever, and the same picker can
  combine `R[p]` and per-slice `Q[p]` for finer rate-distortion
  control.
